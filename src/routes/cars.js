...
router.post('/addcar', jwtCheck, jsonParser, async (req, res) => {
    ...
    var user = await db.query("select user_id, current_car from users where user_id = $1", [req.headers.user_id]);
    ...
    var car = await db.query("insert into cars(user_id, name, year, make, model, picture, milage) values($1, $2, $3, $4, $5, $6, $7) returning car_id;", 
...
router.get('/getcurrentcar', jwtCheck, async(req, res) => {
    ...
    var usersCurrentCar = await db.query("select current_car from users where user_id = $1", [req.headers.user_id]);

    if(usersCurrentCar.rows.length == 1){
        var currentCar = await db.query("select * from cars where car_id = $1 and user_id = $2", [usersCurrentCar.rows[0].current_car, req.headers.user_id]);
